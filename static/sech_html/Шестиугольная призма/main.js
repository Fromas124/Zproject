...
// Функция создания шестиугольной призмы
function createHexagonalPrism() {
    ...
    // Центры оснований
    vertices.push(0, height, 0);  // центр верхнего основания (индекс 12)
    vertices.push(0, 0, 0);       // центр нижнего основания (индекс 13)
    
    // Боковые грани
    for (let i = 0; i < segments; i++) {
        const next = (i + 1) % segments;
        indices.push(i, i + segments, next);
        indices.push(next, i + segments, next + segments);
    }
    
    // Верхнее основание (треугольники от центра)
    for (let i = 0; i < segments; i++) {
        const next = (i + 1) % segments;
        indices.push(12, i, next);
    }
    
    // Нижнее основание (треугольники от центра)
    for (let i = 0; i < segments; i++) {
        const next = (i + 1) % segments;
        indices.push(13, i + segments, next + segments);
    }
