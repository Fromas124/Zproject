...
// Функция создания треугольной призмы
function createTriangularPrism() {
    ...
    // ВЕРШИНЫ 0-2: верхнее основание (треугольник)
    vertices.push(0, height, radius);           // 0: верхняя передняя
    vertices.push(radius * 0.866, height, -radius * 0.5);  // 1: верхняя правая
    vertices.push(-radius * 0.866, height, -radius * 0.5); // 2: верхняя левая
    
    // ВЕРШИНЫ 3-5: нижнее основание (треугольник)
    vertices.push(0, 0, radius);                // 3: нижняя передняя
    vertices.push(radius * 0.866, 0, -radius * 0.5);       // 4: нижняя правая
    vertices.push(-radius * 0.866, 0, -radius * 0.5);      // 5: нижняя левая
    
    // Боковые грани (прямоугольники из двух треугольников)
    // Грань 0-1-4-3
    indices.push(0, 1, 4);
    indices.push(0, 4, 3);
    ...
    prism = new THREE.Mesh(geometry, prismMaterial);
    prism.position.y = 0;
    prism.castShadow = true;
    prism.receiveShadow = true;
    prism.userData.type = 'prism';
    prism.userData.clickable = true;
...
function onMouseClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        for (const inter of intersects) {
            ...
            if (obj.parent && obj.parent.userData.type === 'prism') {
                intersect = inter;
                break;
            } else if (obj.userData.type === 'prism' || obj.userData.type === 'sphere') {
                intersect = inter;
                break;
            }
        }
        ...
        if (userData.type === 'prism' && spheres.length < 3) {
