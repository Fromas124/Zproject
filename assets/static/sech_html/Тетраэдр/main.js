...
function createTetrahedron() {
    ...
    // Вершина 1: передняя правая
    vertices.push(radius, 0, radius);
    
    // Вершина 2: передняя левая
    vertices.push(-radius, 0, radius);
    
    // Вершина 3: задняя
    vertices.push(0, 0, -radius);
    
    // Грани тетраэдра (4 грани)
    indices.push(0, 1, 2);  // передняя грань
    indices.push(0, 2, 3);  // левая грань
    indices.push(0, 3, 1);  // правая грань
    indices.push(1, 3, 2);  // нижняя грань
    ...
    tetrahedron = new THREE.Mesh(geometry, tetrahedronMaterial);
    tetrahedron.position.y = 0;
    tetrahedron.castShadow = true;
    tetrahedron.receiveShadow = true;
    tetrahedron.userData.type = 'tetrahedron';
    tetrahedron.userData.clickable = true;
...
function onMouseClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        for (const inter of intersects) {
            ...
            if (obj.parent && obj.parent.userData.type === 'tetrahedron') {
                intersect = inter;
                break;
            } else if (obj.userData.type === 'tetrahedron' || obj.userData.type === 'sphere') {
                intersect = inter;
                break;
            }
        }
        ...
        if (userData.type === 'tetrahedron' && spheres.length < 3) {
